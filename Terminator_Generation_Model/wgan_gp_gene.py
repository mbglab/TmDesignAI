...
#
#    This script is used for the E. coli Terminator Generation Model. 
#    Revised by Jie Li, Lin-Feng Wu and Bin-Guang Ma, 2025-03.
...
from utils.utils import plot_losses, decode_one_seq

from models import *

class WGAN_LangGP():
    def __init__(self, batch_size=32, lr=0.0001, num_epochs=200, seq_len = 40, data_dir='./data/sequence_data_final.txt', \
        run_name='test', hidden=256, d_steps = 5):
        self.hidden = hidden
    ...
    def train_model(self, load_dir):
        ...
        for epoch in range(self.n_epochs):
            ...
            for idx in range(n_batches):
                ...
                if counter % 10 == 9:
                    #summary_str = 'Iteration [{}/{}] - loss_d: {}, loss_g: {}, w_dist: {}, grad_penalty: {}'.format(i, total_iterations, (d_err.data).cpu().numpy(),(g_err.data).cpu().numpy(), ((d_real_err - d_fake_err).data).cpu().numpy(), gp_np)
                    summary_str = 'Iteration [{}/{}] - loss_d: {}, loss_g: {}, w_dist: {}, grad_penalty: {}'.format(counter, total_iterations, (d_err.data).cpu().numpy(),(g_err.data).cpu().numpy(), ((d_real_err - d_fake_err).data).cpu().numpy(), gp_np)
                    print(summary_str)
...
def main():
    ...
    #parser.add_argument("--run_name", default= "zzz", help="Name for output files (checkpoint and sample dir)")
